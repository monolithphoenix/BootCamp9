 ...
 */
const getUsersByGender = (arr, gender) => arr.filter(el => el.gender === gender).map(obj => obj.name);
 ...
 */
const getUsersWithAge = (arr, min, max) => arr.filter(obj => obj.age > min && max > obj.age);
...
*/
const getAllUniqueSkills = arr => arr.map(obj => obj.skills).toString().split(',').sort().filter((value, i, self) => self.indexOf(value) === i);
...
*/
const getUserNamesSortedByFriendsCount = arr => arr.sort((a, b) => a.friends.length < b.friends.length ? -1 : 1).slice().map(obj => obj.name);
